...
#num_steps是區間總長
#summary_frequency設定每多少區間print1次data
num_steps = 20001
        ...
        if step >0 and step % summary_frequency == 0:
            ...
            for i in range(len(prediction)):
                if prediction[i][testLabels[i].index(1)] > 0.5:
                    acrc = acrc + 1
            test_accuracy = ((acrc/len(testLabels))*100)
            save_accuracy = test_accuracy
            test_list.append(save_accuracy)
            print("In test data,the accuracy is:%.2f%%"%(test_accuracy))
#####################################
timeB=time.time()
print("time cost:",int(timeB-timeA))
best_test_accuracy_position = test_list.index(max(test_list)) #找最好的模型在List中的位置
best_test_accuracy = train_list[best_test_accuracy_position] #找到最好的模型的Training Loss
print("Best accuracy:%.2f%%"%(max(test_list)))
print("Training Loss:%.4f"%(best_test_accuracy))